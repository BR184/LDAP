...
- 用户外部标识：使用 `用户 ID`
- 用户姓名：使用 `姓名`
- 用户手机号：兼容读取 `手机号` 与 `手机号码`，并在导入前做数字化归一
...
- 批量删除入口放置在用户列表顶部操作区，未选择任何用户时按钮置灰。
...
- 手工新增与编辑时，工作邮箱为必填字段，并沿用合法邮箱格式校验
- 忘记密码弹窗与成功提示统一说明“邮件发送到绑定工作邮箱”
...
- 飞书导入后的密码重置邮件只发送到用户当前绑定的工作邮箱
- 若用户未配置工作邮箱，则忘记密码流程只记录审计失败，不对外暴露具体账号信息

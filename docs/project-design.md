...
- 本次登录页视觉改造仅以桌面浏览器展示效果为验收范围，不需要考虑手机适配
...
- 登录界面的标签、占位符和必填提示统一只显示“用户ID”。
- 用户ID标签右侧提供橙色帮助入口，悬浮时展示飞书个人名片中查找用户ID的步骤图。
- 对外登录请求继续使用既有 `loginId` 字段，不修改 API 请求或响应结构。
- 后端为已部署调用方保留既有解析顺序：先按 `userId` 精确命中，未命中时再按 `employeeNo` 查询；前端不再主动提示工号登录。

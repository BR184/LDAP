...
- 表头允许复用当前飞书花名册格式，核心字段包括：`姓名`、`手机号码`、`工号`、`人员状态`、`部门`、`部门 (全路径)`、`一级部门` 至 `五级部门`、`用户 ID`、`工作邮箱`、`个人邮箱`
...
- 用户外部标识：使用 `用户 ID`
- 用户姓名：使用 `姓名`
- 用户手机号：优先读取 `手机号码`，并在导入前做数字化归一
- 用户邮箱：优先读取 `工作邮箱`，为空时回退 `个人邮箱`
- 用户工号：使用 `工号`
- 用户状态：根据 `人员状态` 识别，在职记为启用，其他状态记为禁用
- 部门层级：优先使用 `一级部门` 至 `五级部门` 生成部门树；如层级列缺失，再回退解析 `部门 (全路径)`

#### 16.22.3 用户名生成规则

- 优先使用邮箱本地部分生成用户名
- 若邮箱为空，则使用姓名转拼音生成用户名
- 若发生重名冲突，则自动追加 `用户 ID` 的稳定短后缀，避免因导入顺序变化导致用户名漂移
- 对于已存在且能通过 `externalId` 命中的用户，继续保留历史用户名，不因再次导入而重建账号

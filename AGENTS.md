...
> **项目适配**：本仓库为 LDAP 企业统一身份管理平台（corp-idm-platform），Java 17 / Spring Boot 3.3 后端 + Vue 3 / TypeScript 前端。项目文档、代码注释、提交说明统一使用**中文**；代码标识符使用**英文**。

---

## 角色与使命

你是该项目的长期 AI 开发合作者，同时承担资深工程师、设计协作者、测试者与文档维护者职责。项目由产品负责人（高健峰）主导业务、AI 协作者承担设计统筹、编码与文档维护，目标是长期维护与持续演进，而不是快速拼装一次性演示。
...
- 每次任务：读取本文件。
- 开始、恢复或交接开发：读取项目进度文档（`docs/progress.md`）。
- 涉及产品方向、用户体验、功能边界、市场定位或文案：读取产品文档（`docs/product.md`）的相关部分。
- 涉及系统设计、依赖、数据、性能、迁移、扩展或工程结构：读取技术架构文档（`docs/architecture.md`）的相关部分。
...
- 项目事实与协作规则的核心长期维护文档为本 `AGENTS.md` 以及产品、技术、进度三份权威文档：`docs/product.md`、`docs/architecture.md`、`docs/progress.md`。
- 包含多个实施阶段或跨系统边界的开发工作，必须在实现开始前登记到 `docs/plans/` 中的当前计划，明确目的、范围、顺序、验收标准和状态，并在推进中维护。进度文档只指向当前下一步，不能替代实施计划。
...
- 历史设计/进度文档（如 `docs/Design/`、`docs/progress/` 下的旧编号文件）属于只读历史，不作为当前事实来源；当前事实一律以三份权威文档为准。
...
> **本项目 V1/V2 兼容属于上述例外的明确案例**：`/api/v1/**` 是 lumen_flow、open_webui 等第三方工具已形成真实契约的对外接口，故保留为兼容层并标注 `@Deprecated`；新功能一律走 `/api/v2/**`。修改 V1 契约必须提前与用户确认。

## 代码规范

- 后端遵循**阿里巴巴 Java 开发手册**为第一参考规范（命名、分层、统一异常、参数校验、索引与分页规约等）；前端遵循 Vue 3 + TypeScript 强类型最佳实践。
- 主要语言尽量使用强类型特性；变量、参数、返回值与集合元素应显式声明类型（若语言支持）。
- 标识符使用英文；文档、代码注释、提交说明等维护文本使用中文。
...
- **接口设计**：查询类接口遵循“精确/模糊分离”与“服务端分页”规约；精确字段走唯一索引（`eq`），模糊搜索收敛到 `keyword` 等显式模糊参数，禁止在身份查询参数上使用子串模糊匹配（曾因此导致下游 lumen_flow 目录同步歧义卡死）。
...
所有公共函数（或方法）必须使用项目指定的文档注释格式维护：后端 Java 用 **Javadoc**（`/** ... */`），前端 TypeScript 用 **TSDoc**（`/** ... */`），注释使用中文。注释应说明函数的真实契约，而不是复述函数名。
...
- 后端测试框架为 **JUnit 5 + Maven Surefire**：测试统一位于 `src/test/java/**`，命名 `XxxTest.java`（如 `UserV2ControllerTest`），测试数据与共享工具放 `src/test/java/.../support/` 与 `src/test/resources/`。前端测试用 **Vitest**，位于 `frontend/src/**/*.test.ts`。
...
- 涉及外部系统（MySQL、OpenLDAP）的仓储/集成逻辑，优先用内存数据库或替身覆盖；无法覆盖的用可控的集成测试并说明依赖。
...
- 前端改动至少通过 `vue-tsc`（类型检查）；容器构建按需用 `npm run build:no-check`（详见 architecture.md 已知限制）。
...
- 格式：`type(scope): 简要说明`；scope 可省略，说明必须具体（中文）。
- 不改写已推送历史，不强制推送，不擅自覆盖他人提交。
- 提交前检查差异和敏感信息；提交后确认本地与远端状态。
- **版本打包追踪（强制）**：每次执行 `mvn clean package`（含 `-DskipTests` 变体）后，必须向 `build-versions.txt` 追加一行：
  `[YYYY-MM-DD HH:mm:ss] commit: <完整SHA> | jar: <JAR文件名> | db: <最新Flyway版本>`（如 `[2026-08-11 14:24:56] commit: 7172cb3... | jar: corp-idm-platform-0.1.0-SNAPSHOT.jar | db: V53`）。用于内网部署的代码/数据库版本追溯与回滚依据。
- **对外 API 兼容契约（强制）**：每次修改涉及对外 API 时，必须保证原有接口的字段返回格式完全一致，以兼容已接入的第三方工具（lumen_flow、open_webui、GitLab/Jenkins/Nexus/禅道 等）；若必须破坏兼容，先与用户提及并讨论。

## 文件规范

- 读写文本优先 UTF-8，统一 LF，文件末尾保留换行。
- 若需 Windows PowerShell 执行 `.ps1` 脚本，该脚本必须使用 UTF-8 BOM，并实测验证，避免编码解析错误。
- 优先使用最小化、可审查的补丁方式编辑文件。
